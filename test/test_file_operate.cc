...
TEST(TestFileCreate, NewFile) {
  ...
  system(cmd.c_str());
  EXPECT_EQ(0, FileOperate().FileCreateEmpty(f));
  EXPECT_TRUE(FileOperate().FileIsExist(f));
  system(cmd.c_str());
}
...
TEST(TestFileCreate, ExistedFile) {
  ...
  system(cmd.c_str());
  EXPECT_EQ(-1, FileOperate().FileCreateEmpty(f));
  cmd = "rm -rf " + f;
  system(cmd.c_str());
}
...
class FileOperateTest : public testing::Test {
 protected:
  void SetUp() override {
    f.FileMkdirsByPath(dir);
    if (0 != f.FileCreateEmpty(dir + fname)) {
      throw std::runtime_error("Create file failed");
    }
  }
  void TearDown() override {
    if (true == f.FileIsExist(dir + fname)) {
...
TEST_F(FileOperateTest, CheckFileExist) {
  EXPECT_TRUE(f.FileIsExist(dir + fname));
}

TEST_F(FileOperateTest, CheckFileNotExist) {
  EXPECT_FALSE(f.FileIsExist(fname));
}

TEST_F(FileOperateTest, WriteJsonToFile) {
  EXPECT_EQ(test_json.dump().length(), f.FileWriteTo(dir + fname, test_json));
  json read_json;
  EXPECT_EQ(test_json.dump().length(), f.FileReadFrom(dir + fname, read_json));
  EXPECT_EQ(test_json, read_json);
}

TEST_F(FileOperateTest, WriteStringToFile) {
  EXPECT_EQ(test_string.length(), f.FileWriteTo(dir + fname, test_string));
  std::string read_string;
  EXPECT_EQ(test_string.length(), f.FileReadFrom(dir + fname, read_string));
...
TEST_F(FileOperateTest, SearchFileWithRegex) {
  ...
  EXPECT_TRUE(f.FileSearchRecursively(fp, regex, dir, 1, pattern));
...
TEST_F(FileOperateTest, SearchFileWithRegexNotExisted) {
  ...
  std::string pattern = "regex";
  EXPECT_FALSE(f.FileSearchRecursively(fp, regex, dir, 1, pattern));
  EXPECT_EQ("", fp);
}
...
TEST_F(FileOperateTest, SearchFileWithRegexRecursively) {
  ...
  int depth = 2;
  std::string pattern = "regex";
  EXPECT_TRUE(f.FileSearchRecursively(fp, regex, path, depth, pattern));
  EXPECT_EQ(dir + fname, fp);
}
...
TEST_F(FileOperateTest, SearchFileWithKeyword) {
  ...
  EXPECT_TRUE(f.FileSearchRecursively(fp, keyword, dir, 1, pattern));
  EXPECT_EQ(dir + fname, fp);
}
...
TEST_F(FileOperateTest, SearchFileWithKeywordNotExisted) {
  ...
  std::string pattern = "keyword";
  EXPECT_FALSE(f.FileSearchRecursively(fp, keyword, dir, 1, pattern));
}
...
TEST_F(FileOperateTest, SearchFileWithKeywordRecursively) {
  ...
  int depth = 2;
  std::string pattern = "keyword";
  EXPECT_TRUE(f.FileSearchRecursively(fp, keyword, path, depth, pattern));
  EXPECT_EQ(dir + fname, fp);
}
...
TEST_F(FileOperateTest, GetFileNameWithoutSuffix) {
  ...
  EXPECT_EQ(name, f.FileGetNameWithoutSuffix(file));
  EXPECT_EQ(fname, f.FileGetNameWithoutSuffix(dir + fname));
}

TEST_F(FileOperateTest, GetFileSuffix) {
  std::string file = "test.txt";
  std::string suffix = "txt";
  EXPECT_EQ(suffix, f.FileGetSuffix(file));
  EXPECT_EQ("", f.FileGetSuffix(fname));
}

TEST_F(FileOperateTest, ListFilesInDir) {
  std::vector<std::string> fl = f.FileListInDir(dir, "file", "");
  EXPECT_EQ(1, fl.size());
